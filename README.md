...
- By default, the value of `server.Options.ClientNetWriteTimeout` is `0`, which leaves writes to a client unbounded and is the behaviour of every release before the option existed. It is left off so that upgrading changes nothing on its own, but a deployment that carries clients which may stop reading should set it: it is what bounds how long one such client can hold its own write lock, and a write that exceeds it stops that client.
...
| OnSelectSubscribers    | Called for every publish once subscribers have been collected for a topic, and before shared subscription subscribers have been selected. Allows receipient modification.                                                                                                                                  |
